import { calculateSoapFormula, SoapIngredient } from '../src/utils/soapCalculations';

describe('Soap Calculations', () => {
  test('Test recipe with citric acid: 250g Kokosöl, 80g Rizinus, 120g Sonnenblume, 100g Shea, 450g Distel, 5% Zitronensäure', () => {
    // Testseife-Rezept
    const ingredients: SoapIngredient[] = [
      { oilId: 'coconut', weight: 250 },     // Kokosöl: SAP 183
    ...
    const expectedAdditionalNaoh = Math.round(50 * 0.624 * 100) / 100;
    expect(result.additionalNaohForCitricAcid).toBe(31.2);
    ...
    console.log(`NaOH for Oils (with 10% superfat): ${expectedNaohForOils}g`);
    console.log(`Citric Acid Weight: ${result.citricAcidWeight}g`);
    console.log(`Additional NaOH for Citric Acid: ${result.additionalNaohForCitricAcid}g`);
    console.log(`Total NaOH: ${result.naohWeight}g`);
    console.log(`Water Weight: ${result.waterWeight}g`);
  });
